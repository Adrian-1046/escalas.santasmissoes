...
const ministrantes = require('./index/funcoesDeBanco/ministrantes')
const datas = require('./index/funcoesDeBanco/datas')
...
async (req, res) => {
  ...
  switch (tela) {
    case 'DomingoManha':
      atualizar_ministro_dm()
      break;
    case 'DomingoNoite':
      atualizar_ministro_dn()
      break;
    case 'Terca':
      atualizar_ministro_t()
      break;
    case 'Quarta':
      atualizar_ministro_q()
      break;
    default:
      break;
  }

  async function atualizar_ministro_dm(){
    try {
      const {
        ministro_dm,
      } = req.body
  
      await ministrantes.update(
        {
          'MINISTRANTE-DM': ministro_dm,
        },
        { where: { 1: 1 } }
      )
    } catch (error) {
  ...
  async function atualizar_ministro_dn(){
    try {
      const {
        ministro_dn,
      } = req.body
  ...
  async function atualizar_ministro_t(){
    try {
      const {
        ministro_t,
      } = req.body
  ...
  async function atualizar_ministro_q(){
    try {
      const {
        ministro_q,
      } = req.body
