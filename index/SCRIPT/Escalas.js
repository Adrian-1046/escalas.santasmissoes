...
function abrirEscalas(solicitado) {
  document.querySelector('.dias-escala').style.visibility = 'hidden';
  let selecionado, tela;
  switch (solicitado) {
    case 'quartaFeira':
      selecionado = escalaQuartaFeira();
      tela = 'Quarta';
      break;
    case 'domingoManha':
      selecionado = escalaDomingoManha();
      tela = 'DomingoManha';
      break;
    case 'domingoNoite':
      selecionado = escalaDomingoNoite();
      tela = 'DomingoNoite';
      break;
    case 'tercaFeira':
      selecionado = escalaTercaFeira();
      tela = 'Terca';
      break;
    default:
...
buscarDadosDoServidorEscalas(tela);
...
function buscarDadosDoServidorEscalas(tela) {
  try {
    mostrarLoading();
    fetch(`/inicio/escalas/${tela}`, { timeout: 50000 })
      .then(response => {
      ...
      .then(dados => {
        ...
        for (let i = 0; i < totalLinhas; i++) {
          ...
          document.getElementById(`S${i + 1}_Ministro`).innerText =
            dadosArray[i * colunas + 1];
          document.getElementById(`S${i + 1}_Violão`).innerText =
            dadosArray[i * colunas + 2];
          document.getElementById(`S${i + 1}_Baixo`).innerText =
            dadosArray[i * colunas + 3];
          document.getElementById(`S${i + 1}_Teclado`).innerText =
            dadosArray[i * colunas + 4];
          document.getElementById(`S${i + 1}_Bateria`).innerText =
            dadosArray[i * colunas + 5];
          document.getElementById(`S${i + 1}_Guitarra`).innerText =
            dadosArray[i * colunas + 6];
          document.getElementById(`S${i + 1}_Back_1`).innerText =
            dadosArray[i * colunas + 7];
          document.getElementById(`S${i + 1}_Back_2`).innerText =
            dadosArray[i * colunas + 8];
          document.getElementById(`S${i + 1}_Back_3`).innerText =
            dadosArray[i * colunas + 9];
          document.getElementById(`S${i + 1}_Mídia`).innerText =
            dadosArray[i * colunas + 10];
          document.getElementById(`S${i + 1}_Som`).innerText =
