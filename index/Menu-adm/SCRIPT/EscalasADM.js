...
function gravarEscalaADM(tela) {
  const updateData = index => {
    const sData = encodeURIComponent(
      document.getElementById(`S${index}_data`).value.trim()
    )
  ...
  const updateMinistrante = index => {
    const sMinistro = encodeURIComponent(
      document.getElementById(`S${index}_Ministro`).innerText.trim()
    )

    console.log(sMinistro);
    const ministrante = `ministroS${index}=${sMinistro}`

      fetch(`/atualizar-ministrantes-ADM/${tela}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
    ...
    fetch(`/inicio/escalas-ADM/${tela}`, { timeout: 50000 })
      ...
      .then(dados => {
        ...
        for (let i = 0; i < totalLinhas; i++) {
          document.getElementById(`S${i + 1}_data`).value =
