...
mostrarLoading()
  ...
  const sData = encodeURIComponent(
    document.getElementById(`data${dia}`).value.trim()
  );

  const sMinistante = encodeURIComponent(
    document.getElementById(`ministrante${dia}`).innerText.trim()
  );
  ...
  const bodyData = `DATA=${sData}&MINISTRANTE=${sMinistante}&MUSICA=${sMusica}&LINK=${sLink}&OBS=${sObs}`;
